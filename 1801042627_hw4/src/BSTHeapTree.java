...
public class BSTHeapTree <E extends Comparable<E>>{
	...
	/*
	 * garbage codes which I wrote when I tried to find element to put to the position of the just removed element 
	 */
	
	/*
	private void removeAndReturnBiggestChild(MyBst.Node<myHeap<Cup<E>>>  node){
		while(node != null && node.getRight() != null ) {
			node.getData().add(node.getRight().getData().remove());
			node = node.getRight();
		}
		if(node == null || MyBst.isLeaf(node)) return;
		
		if(node.getLeft() != null) {
			node.getData().add(node.getLeft().getData().remove());
			node = node.getLeft();
		}
		if(!MyBst.isLeaf(node)) {
			 removeAndReturnBiggestChild(node);
		}else {
			if(node.getData().size() == 0) {
				tree.delete(node);
	...
	private void removeAndReturnBiggestChild2(MyBst.Node<myHeap<Cup<E>>>  node){
		while(node != null && node.getRight() != null ) {
			node.getData().add(node.getRight().getData().remove());
			node = node.getRight();
		}
		...
		if(!MyBst.isLeaf(node)) {
			 removeAndReturnBiggestChild2(node);
		}else {
			if(node.getData().size() == 0) {
				tree.delete(node);
	...
	private Cup<E> removeAndReturnLastElement(MyBst.Node<myHeap<Cup<E>>> node){
		MyBst.Node<myHeap<Cup<E>>> tmp = getLeafNode(node);
		myHeap<Cup<E>> heap = tmp.getData();
		if(heap.size() == 1) {
			tree.delete(heap);
			return heap.get(0);
		}else {
	...
	private MyBst.Node<myHeap<Cup<E>>> getLeafNode(MyBst.Node<myHeap<Cup<E>>> temp){
		MyBst.Node<myHeap<Cup<E>>> node = temp;
		boolean isThereRight;
		while( (isThereRight = (node.getRight()!= null)) ||  (node.getLeft() != null)) {
			if(isThereRight) {
				node = node.getRight();
			} else {
				node = node.getLeft();
		...
		// came to the leaf node
		if(!MyBst.isLeaf(node)) {
			throw new NullPointerException();
		}
		return node;
	}
	private void fillTheGap(MyBst.Node<myHeap<Cup<E>>> node) {
		if(node == null)
			return;
		MyBst.Node<myHeap<Cup<E>>> temp = node;
		boolean isThereLeft;
		boolean isThereRight;
		while( (isThereLeft = (node.getLeft()!= null)) || (isThereRight = (node.getRight() != null))) {
			if(isThereLeft) {
				node = node.getLeft();
			} else {
				node = node.getRight();
		...
		myHeap<Cup<E>> heap = node.getData(); // there is no empty node so heap is not null
		Cup<E> cup = heap.get(heap.size()-1);
		if(heap.size() == 1 ) { // last element is about to be removed
			myHeap<Cup<E>> deleted =  tree.delete(heap);
			if(deleted == null) {
				throw new NullPointerException();
			}
		}
		heap.remove(heap.size()-1);
		temp.getData().add(cup);
