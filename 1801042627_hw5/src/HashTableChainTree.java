 ...
 * @param <V> Value
 */
public class HashTableChainTree<K, V extends Comparable<V> > implements KWHashMap<K, V> {
	private TreeSet<Entry<K, V>>[] table;
	private static final int INITIAL_SIZE = 11;
	private int size;
	private static final double LOAD_THRESHOLD = 3.0; 
	/**
	 * constructs the emprt hashmap
	 */
	@SuppressWarnings("unchecked")
	public HashTableChainTree() {
		//table = (TreeSet<Entry<K,V>>[]) new Object[INITIAL_SIZE];
		table = new TreeSet[INITIAL_SIZE];
		size = 0;
	}
	/**
	 ...
	 */
	private void rehash() {
		TreeSet<Entry<K, V>>[] newTable = new TreeSet[table.length*2+1];
		for(TreeSet<Entry<K, V>> list :table) {
			...
			for(Entry<K, V> entry : list) {
				...
				if(newTable[index] == null) {
					newTable[index] = new TreeSet<Entry<K, V>>();
				}
	...
	/**
	 * if the TreeSet in the given index has the given key change its value
	 * @param index index of the TreeSet which holds the value
	...
	public static void main(String[] args) {
		HashTableChainTree<String, String> hash = new HashTableChainTree<String,String>();
		 ...
		 * for 0 to 50 it will remove 50 element
		 */
		for(int i=-10;i<50;i++) {
